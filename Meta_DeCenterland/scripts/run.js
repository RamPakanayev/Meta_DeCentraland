...
const main = async (cb) => {
  try {
    ...
    let txn1 = await flats.safeMint("URI1")
    ...
    let txn2 = await flats.safeMint("URI1")
    ...
    let txn3 = await flats.safeMint("URI1")
    ...
    await marketplace.buyNft(flats.address, tokenId1, {value: 1})
    await marketplace.buyNft(flats.address, tokenId2, {value: 1})
    await logNftLists(marketplace)
